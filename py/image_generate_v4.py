    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                ...
                print(f"{log_prefix} ⚠️ [第{attempt}/{retries}次] 下载失败: {e}，{wait:.1f}秒后重试...")
                time.sleep(wait)
            else:
    ...
    for attempt in range(1, max_api_retries + 1):
        ...
        try:
            ...
            # 2. 鉴权失败/额度不足 (401/403)
                ...
                print(f"{log_prefix} ❌ [密钥失效/无额度] (HTTP {response.status_code}) 当前 Key 标记为失效，不再放回队列！")
                is_key_valid = False
            ...
            # 3. 频控限流 (429)
                ...
                print(f"{log_prefix} ⚠️ [触发频控 (429)] 等待 {wait_time:.1f} 秒后换 Key 重试...")
                time.sleep(wait_time)

            # 4. 其他服务端错误 (5xx)
            else:
                wait_time = 2 ** attempt
                print(f"{log_prefix} ⚠️ [服务端错误 (HTTP {response.status_code})] 将在 {wait_time} 秒后重试...")
                time.sleep(wait_time)

        except requests.exceptions.RequestException as e:
            wait_time = 2 ** attempt
            print(f"{log_prefix} ⚠️ [网络错误 (尝试 {attempt}/{max_api_retries})] 请求 API 失败: {e}，将在 {wait_time} 秒后重试...")
            time.sleep(wait_time)
        except Exception as e:
